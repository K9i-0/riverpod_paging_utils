...
class SampleRepository {
  Future<(List<SampleItem> items, bool hasMore)> getByPage({
    ...
    final items =
        _db
            ...
            .mapRecord((id, name) => SampleItem(id: id.toString(), name: name))
            .toList();
  ...
  Future<(List<SampleItem> items, String? nextCursor)> getByCursor(
    ...
    print(
      '[SampleRepository] getByCursor($cursor) delay: ${stopwatch.elapsedMilliseconds}ms',
    );

    final items =
        _db
            .sublist(int.parse(cursor ?? '0'), int.parse(cursor ?? '0') + 30)
            .mapRecord((id, name) => SampleItem(id: id.toString(), name: name))
            .toList();
    ...
    return (items, exist ? nextCursor : null);
  }
...
final _db = List.generate(150, (index) => (index + 1, _randomNames.fullName()));
